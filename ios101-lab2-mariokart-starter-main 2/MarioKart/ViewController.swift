...
class ViewController: UIViewController,
  ...
  private func decipher(mysteryBox: MysteryBox) {
      guard let effectDictionary = mysteryBox.mysteryEffect as? [String: String]
      else{
          print("expected dictionary type")
          return
          
      }
      guard let effect = effectDictionary["effect"]
      else{
          print("expected type string")
          return
          
      }
      print(effect)
      if effect == "translate"{
          translate(kart: kartView1,
                    by: view.bounds.width)
      }
      if effect == "scale"{
          scale(kart: kartView1)
      }
  ...
  // Exercise 4: Implement applyNumKartsSetting to show the correct number of karts
    func applyNumKartsSetting(_ settings: [String : Any]) {
        guard let numKarts = settings["numKarts"] as? Int
        else {return }
  ...
  func applyKartSizeSetting(_ settings: [String : Any]) {
      guard let kartSizeMultiplier = settings["kartSize"] as? Int
      else {
          print("no kart type")
                return }
